...
public class S3OperationServiceImpl implements S3OperationService {
	...
	@Override
	public List<S3Operation> getBucketfiles(String bucketName) {
		if (!amazonS3.doesBucketExistV2(bucketName)) {
			log.error("No Bucket Found");
			return null;
		}
		return amazonS3.listObjectsV2(bucketName).getObjectSummaries().stream()
				.map(file -> new S3Operation(file.getKey(), file.getSize(), file.getETag()))
	...
	@Override
	public S3Operation downloadFile(String bucketName, String fileName) {
		if (!amazonS3.doesBucketExistV2(bucketName)) {
			log.error("No Bucket Found");
			return null;
		}
		...
		try {
			S3Operation.setFile(FileCopyUtils.copyToByteArray(inputStream));
			S3Operation.setFileName(s3object.getKey());
			return S3Operation;
		} catch (Exception e) {
			return null;
		}
